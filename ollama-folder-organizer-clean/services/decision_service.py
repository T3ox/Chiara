...
def action_for_review(scanned_file: ScannedFile, reason: str) -> FileAction:
    new_path = unique_path(os.path.join(scanned_file.root_dir, REVIEW_FOLDER, scanned_file.name))
    return FileAction(
        action="move_to_review",
        old_path=scanned_file.path,
        new_path=new_path,
        new_name=scanned_file.name,
...
def decide_action(
    ...
    if classification.confidence == "low":
        return action_for_review(scanned_file, classification.reason or "Classificazione incerta.")
    ...
    if classification.target_folder == REVIEW_FOLDER or is_uncertain_name(classification.new_name):
        return action_for_review(scanned_file, classification.reason or "Nome o destinazione incerti.")
